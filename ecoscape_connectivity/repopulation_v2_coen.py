...
class RandomPropagate(nn.Module):
    ...
    def __init__(self, habitat, terrain, num_spreads=100, spread_size=1, device=None, random_coef=0.5, x_coef=0.25):
        """
    ...
    def forward(self, seed):
        ...
        for i in range(self.num_spreads):
            ...
            x = x * (self.min_transmission + (1. - self.min_transmission) * random_matrix)
            # Then, we propagate.
            x = self.spreader(x)
            # Second randomization. 
            x = x * (self.random_coef * random_matrix + self.x_coef * x > 0.5)
            x = x * self.goodness
...
def analyze_tile_torch(
    ...
    def f(habitat, terrain):
        ...
        if not callable(dispersal):
            ...
            repopulator = analysis_class(hab, ter, num_spreads=num_spreads, spread_size=gap_crossing + 1, device=device, random_coef=random_coef, x_coef=x_coef).to(device)
        for i in range(num_batches):
            ...
            if callable(dispersal):
                repopulator = analysis_class(hab, ter, num_spreads=num_spreads, spread_size=spread_size, device=device, random_coef=random_coef, x_coef=x_coef).to(device)
            # Creates the seeds.
