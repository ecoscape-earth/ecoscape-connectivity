...
class DiagonalPropagate(nn.Module):
    """
    ...
    def __init__(self, habitat, terrain, num_spreads=100, spread_size=1, diagonal_coef=0.9, device=None, random_coef=1, x_coef=0):
        """
    ...
    def forward(self, seed):
        ...
        for i in range(self.num_spreads):
            ...
            x = x * (self.random_coef * torch.rand_like(x) + self.x_coef * x > 0.5)
            x *= self.goodness
...
def analyze_tile_torch(
        device=None,
        analysis_class=DiagonalPropagate,
        seed_density=4.0,
    ...
    def f(habitat, terrain):
        ...
        if not callable(dispersal):
            ...
            repopulator = analysis_class(hab, ter, num_spreads=num_spreads, 
                                         spread_size=gap_crossing + 1, diagonal_coef=diagonal_coef, 
                                         random_coef=random_coef, x_coef=x_coef, device=device).to(device)
        for i in range(num_batches):
            ...
            if callable(dispersal):
                repopulator = analysis_class(hab, ter, num_spreads=num_spreads, spread_size=spread_size, 
                                             diagonal_coef=diagonal_coef, random_coef=random_coef,
                                             x_coef=x_coef, device=device).to(device)
            # Creates the seeds.
